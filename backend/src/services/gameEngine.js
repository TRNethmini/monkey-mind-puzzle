...
import { prefetchQuestions } from './bananaService.js';
...
export async function initializeGame(lobbyCode) {
  try {
...
export function getCurrentQuestion(lobbyCode) {
...
export async function nextQuestion(lobbyCode) {
  const gameState = activeGames.get(lobbyCode);
...
export async function endGame(lobbyCode) {
  try {
    const gameState = activeGames.get(lobbyCode);
...
export function getGameState(lobbyCode) {
  return activeGames.get(lobbyCode);
}

// Stop games that run longer than expected
export function checkGameTimeout(lobbyCode) {
  const gameState = activeGames.get(lobbyCode);
...
export async function forceEndGame(lobbyCode) {
  const gameState = activeGames.get(lobbyCode);
...
export async function cleanupInactiveGames() {
  const now = Date.now();
