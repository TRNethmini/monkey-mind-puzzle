...
import { generateQuestionId } from '../utils/codeGenerator';

// Handles pulling puzzles from the Banana API with simple fallbacks

interface BananaApiResponse {
  question: string; // URL to puzzle image
  answer: number | string; // Correct answer (numeric or letter)
  solution?: number; // Alternative field name
}

// Backup questions used when the Banana API is down
const FALLBACK_QUESTIONS: Omit<IQuestion, 'id' | 'timeLimit'>[] = [
  {
    type: 'text',
...
class QuestionCache {
  private cache: Map<string, IQuestion[]> = new Map();
  private readonly maxAge = 3600000; // 1 hour

  set(key: string, questions: IQuestion[]): void {
    this.cache.set(key, questions);
    setTimeout(() => this.cache.delete(key), this.maxAge);
  }

  get(key: string): IQuestion[] | undefined {
...
async function fetchSingleBananaPuzzle(): Promise<IQuestion> {
  ...
  try {
    ...
    const response = await axios.get<BananaApiResponse>(apiUrl, {
      timeout: 5000,
      headers: {
...
async function fetchFromBananaApi(count: number, _difficulty?: string): Promise<IQuestion[]> {
...
function getFallbackQuestions(count: number, difficulty?: string): IQuestion[] {
