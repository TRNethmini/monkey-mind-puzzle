...
export function setupSocketHandlers(io: Server): void {
  io.on('connection', async (socket: AuthSocket) => {
    ...
    socket.on('joinLobby', async (data: JoinLobbyPayload) => {
      try {
    ...
    socket.on('updateLobbySettings', async (data: any) => {
      try {
        if (!socket.userId) {
          socket.emit('error', { message: 'Not authenticated' });
          return;
    ...
    socket.on('startGame', async (data: { code: string }) => {
      try {
        if (!socket.userId) {
          socket.emit('error', { message: 'Not authenticated' });
          return;
    ...
    socket.on('submitAnswer', async (data: SubmitAnswerPayload) => {
      try {
        if (!socket.userId || !socket.userName) {
          socket.emit('error', { message: 'Not authenticated' });
          return;
    ...
    socket.on('requestNextQuestion', async (data: { code: string }) => {
      try {
        const { code } = data;
        const lobby = await Lobby.findOne({ code: code.toUpperCase() });
...
async function sendCurrentQuestion(io: Server, lobbyCode: string): Promise<void> {
  try {
...
async function sendNextQuestion(io: Server, lobbyCode: string): Promise<void> {
  try {
